...
#     查询当前分支最近一次相关 CI
# - `bash scripts/check_ci.sh --branch some-branch`
#     查询指定分支最近一次相关 CI
# - `bash scripts/check_ci.sh --repo owner/name --branch some-branch`
#     查询指定仓库、指定分支最近一次相关 CI
# - `bash scripts/check_ci.sh --json`
...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
用法：
  bash scripts/check_ci.sh [--repo owner/name] [--branch branch-name] [--json]
  bash scripts/check_ci.sh --watch [--interval 60] [--timeout 300] [--repo owner/name] [--branch branch-name]
