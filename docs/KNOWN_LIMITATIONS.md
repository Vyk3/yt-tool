...
### 影响
启动后在应用内切换语言，Sparkle 更新弹窗仍显示启动时的语言。下次启动应用后 Sparkle 弹窗会使用新语言。仅影响 Sparkle 弹窗（检查更新 / 发现新版本），应用内所有其他 UI 均实时切换。

### 后续建议
此为 macOS 框架限制，非程序 bug。影响范围窄（仅更新弹窗）、触发概率低（需要切换语言后立即检查更新），暂不处理。
