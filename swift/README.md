...
- 已补齐：播放列表逐条格式映射（`Per-item mapping`），语法 `itemIndex=formatSelector;itemIndex=formatSelector`
- 已补齐：整列表字幕策略（manual/auto + lang）
- 已补齐：整列表片段策略（固定片段范围转换为 `--download-sections *<range>`）
...
4. 手工 smoke 建议命令与输入：
   - URL：任意包含 `list=` 的 YouTube 播放列表 URL
   - Playlist mode：`Whole playlist: best video`（或 `best audio`）
   - Playlist formats：`Per-item mapping`
   - Per-item map：`1=137+140;2=136+140`
   - Playlist subtitles：`Manual subtitles`，Subtitle language：`en`
   - Playlist segments：`Fixed time range`，Time range：`00:30-01:00`
