...
- 播放列表最小策略已完成：播放列表 URL 现支持 `Only first item`、`Whole playlist: best video`、`Whole playlist: best audio` 三种模式；整列表视频模式额外支持 `Best compatibility` / `Prefer higher quality` 两种整体质量策略；整列表模式仍跳过逐条格式选择并直接下载全部条目

当前验证结果：
- `scripts/test/swift_test.sh`：32 个测试通过
...
- 播放列表手工验收：播放列表 URL 会显示模式选择器，默认 `Only first item`；切换到 `Whole playlist` 模式后，`Probe first item` 收起，格式区改为自动下载说明，`Download` 入口按模式放开；`Whole playlist: best video` 下会出现 `Video quality`，可在 `Best compatibility` 和 `Prefer higher quality` 之间切换，切到 `Whole playlist: best audio` 后该行会消失

当前剩余重点：
- 正式文档与 backlog 口径继续同步，避免 Swift 当前能力和 limitation 描述再次漂移
- 播放列表模式目前仍是最小版：不支持逐条格式选择、整列表字幕或整列表片段策略；整列表质量策略当前只覆盖 `Whole playlist: best video`
- 持久化输出目录现在会在启动和下载前校验是否仍然存在且可用；如目录已失效，UI 不再把它当成可下载状态
- 如需继续做新功能，优先从播放列表整体策略这类独立切片进入，而不是继续微调下载面板视觉
...
4. 从 backlog 选择新的独立功能切片，而不是继续叠加同一块下载区 UX
