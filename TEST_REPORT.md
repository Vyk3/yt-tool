...
> 说明：本文中的联调明细（IT-xx 与 55/55 统计）是 2026-03-30 的历史快照。
> 当前仓库状态请以 CI 工作流与“最新回归快照”小节为准。

## 测试结论
当前版本已完成核心联调与自动化测试，主流程可用，异常路径可控，达到发布前收口条件。
...
- 测试日期: 2026-03-30
- 测试视频: `AJR - World's Smallest Violin`
- 测试 URL: `https://www.youtube.com/watch?v=PEnJbjBuxnw`
...
| # | 用例 | 测试位置 | 状态 |
|---|------|----------|------|
| 1 | `ask_download_type()` 非法值后重试 | `test_ui.py::TestAskDownloadType::test_invalid_then_valid` | 通过 |
| 2 | `menu_select()` 输入 0 返回 None | `test_ui.py::TestMenuSelect::test_skip_returns_none` | 通过 |
| 3 | `ensure_dir()` 文件路径报错 | `test_path_utils.py::TestEnsureDir::test_file_path_raises` | 通过 |
| 4 | `download_video()` 空参数返回失败 | `test_downloader.py::TestDownloadVideo::test_empty_url_fails` | 通过 |
| 5 | `detect()` 非 0 返回码抛 RuntimeError | `test_format_detector.py::TestDetect::test_ytdlp_nonzero_raises` | 通过 |
| 6 | `main()` detect 失败时返回 1 | `test_main_flow.py::TestMainFlow::test_detect_failure_returns_1` | 通过 |
...
### 已知限制
当前已知限制见 `KNOWN_LIMITATIONS.md`，不作为本轮联调失败项。
